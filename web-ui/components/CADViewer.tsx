...
import { AlertCircle, Share2, Download, ChevronDown, Layers, Box, Loader2, Database } from 'lucide-react';
...
export type CADViewerRef = {
    rebuild: () => void;
    exportModel: (format: 'stl' | 'dxf' | 'csg' | 'step', dxfMode?: 'silhouette' | 'section' | 'blueprint') => Promise<ArrayBuffer>;
    respawn: () => void;
};
...
export const CADViewer = forwardRef<CADViewerRef, CADViewerProps>(function CADViewer(
    ...
    const handleExport = useCallback(
        async (format: 'stl' | 'dxf' | 'csg' | 'step', dxfMode?: 'silhouette' | 'section' | 'blueprint') => {
            if (!code) return;
                ...
                <div className="absolute top-4 right-4 z-10 flex items-center gap-2">
                    ...
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end" className="w-48 border-[#3C3C3C] bg-[#252526] p-1 shadow-2xl rounded-md">
                            ...
                            {/* --- NEW CSG BUTTON --- */}
                            <DropdownMenuItem onClick={() => handleExport('csg')} className="text-[11px] text-[#D4D4D4] focus:bg-[#007ACC] rounded-md py-1.5 cursor-pointer">
                                <div className="flex items-center gap-2.5">
                                    <div className="flex size-5 items-center justify-center rounded bg-purple-500/20">
                                        <Database size={11} className="text-purple-500" />
                                    </div>
                                    <div className="flex flex-col">
                                        <span>Math Kernel</span>
                                        <span className="text-[9px] text-white/60">.CSG Blueprint</span>
                                    </div>
                                </div>
                            </DropdownMenuItem>
                            ...
                            <DropdownMenuItem onClick={() => handleExport('step')} className="text-[11px] text-[#D4D4D4] focus:bg-[#007ACC] rounded-md py-1.5 cursor-pointer">
                                <div className="flex items-center gap-2.5">
                                    ...
                                    <div className="flex flex-col">
                                        <span>Machining Solid</span>
                                        <span className="text-[9px] text-white/60">.STEP B-Rep</span>
                                    </div>
                                </div>
                            </DropdownMenuItem>
