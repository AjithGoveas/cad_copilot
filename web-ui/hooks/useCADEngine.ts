...
export function useCADEngine({
    ...
    const exportModel = useCallback(async (
        ...
        return new Promise<ArrayBuffer>((resolve, reject) => {
            // Set up our request listener
            pendingRequestsRef.current.set(requestId, async (data) => {
                if (data.type === 'exported') {
                    // ── STEP INTERCEPT PIPELINE ──
                    if (format === 'step') {
                        try {
                            const response = await fetch('/api/v1/export/step', {
                                method: 'POST',
                                headers: { 'Content-Type': 'application/octet-stream' },
                                body: data.data // Raw CSG buffer from the worker
                            });

                            if (!response.ok) {
                                throw new Error(`Backend FreeCAD engine failed: ${response.statusText}`);
                            }
                        ...
                        // Standard formats resolve directly
                        resolve(data.data);
                    }
            ...
            // Command the worker. If format is 'step', tell it to output 'csg' internally.
            const workerFormat = format === 'step' ? 'csg' : format;
            worker.postMessage({ 
                type: 'export', 
                script, 
                format: workerFormat, 
